    ...
    for user in users_ref:
        ...
        if data.get("verification_token") == token:
            db.collection("users").document(user.id).update(
                {"verified": True, "verification_token": firestore.DELETE_FIELD}
            )
...
def save_profile():
    ...
    try:
        encrypted_profile = {
            key: encrypt_data(str(value)) for key, value in profile.items()
        }
...
def profile():
    ...
    try:
        ...
        if profile_doc.exists:
            ...
            profile = {
                key: decrypt_data(value) for key, value in encrypted_profile.items()
            }
        else:
            profile = None
    except Exception as e:
...
def chat():
    ...
    try:
        ...
        profile = (
            {key: decrypt_data(value) for key, value in profile_doc.to_dict().items()}
            if profile_doc.exists
            else {}
        )
        ...
        if history:
            history_text = "\n".join(
                [
                    f"User: {decrypt_data(msg['user'])}\nBot: {decrypt_data(msg['bot'])}"
        ...
        history.append(
            {"user": encrypt_data(user_input), "bot": encrypt_data(bot_reply)}
        )
...
def community():
    ...
    try:
        ...
        username = (
            decrypt_data(profile.get("username"))
        ...
        for doc in posts_ref.stream():
            ...
            post_data["replies"] = [
                {
                    "content": decrypt_data(r.to_dict()["content"]),
                    "visible_username": decrypt_data(r.to_dict()["visible_username"]),
                }
